...
1. **Fyne UI (`main.go` entry + `internal/ui/{model,viewmodel,view}`)** — MVVM split. `main.go` only constructs the app/window and wires `viewmodel.New(prefs)` to `view.Build(window, vm)`. `model` owns the `Config` struct, prefs I/O, parsers, and `RunnerConfig`. `viewmodel.Download` holds Fyne `binding.*` state for every form field plus `Status`/`LogText`/`ProgressValue`/`ProgressDetail`/`ProgressVisible`/`Running`, drives the download goroutine, and exposes `OnError`/`OnInfo` hooks for dialogs. `view.Build` constructs the widget tree and binds widgets via `widget.NewEntryWithData` / `NewCheckWithData` / `entry.Bind` / `binding.NewDataListener`. All UI mutations from background goroutines must go through `fyne.Do(...)` — see `appendLog` and `handleProgress` in `viewmodel.go` for the pattern. Window title pulls `Version`/`Build` from `a.Metadata()` (sourced from `FyneApp.toml`); there is no ldflag injection.

2. **`internal/luxrunner`** — thin orchestrator. `Run(Config)` normalizes inputs, calls `request.SetOptions` (global lux state — be aware concurrent `Run`s would race on it), iterates URLs through `extractors.Extract`, and hands each result to a `luxdownloader.Downloader`. `register.go` is the side-effect import list for every lux extractor the app supports; adding a new site means adding an import there. `luxrunner` re-exports the `ProgressEvent` / `ProgressPhase` types from `luxdownloader` so the UI layer doesn't depend on the lower package directly.
