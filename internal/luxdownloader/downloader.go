...
func (downloader *Downloader) writeFile(url string, file *os.File, headers map[string]string) (int64, error) {
	...
	written, copyErr := io.Copy(&progressWriter{writer: file, tracker: downloader.progress}, res.Body)
...
func (downloader *Downloader) save(part *extractors.Part, refer, fileName string) error {
	...
	if exists && fileSize == part.Size {
		downloader.progress.add(fileSize)
		return nil
	}
	...
	if tempFileSize > 0 {
		...
		downloader.progress.add(tempFileSize)
	} else {
...
func (downloader *Downloader) multiThreadSave(dataPart *extractors.Part, refer, fileName string) error {
	...
	if tmpExists {
		if tmpFileSize == dataPart.Size {
			downloader.progress.add(dataPart.Size)
	...
	if savedSize > 0 {
		downloader.progress.add(savedSize)
...
func (downloader *Downloader) Download(data *extractors.Data) error {
	...
	downloader.progress = newProgressTracker(ProgressEvent{
		...
		Message:  "downloading",
		Total:    stream.Size,
	}, downloader.option.OnProgress, downloader.option.ProgressThrottle)
	...
	if downloader.option.UseAria2RPC {
		downloader.progress.emit(true)
		err := downloader.aria2(title, stream)
		if err != nil {
			return err
		}
		downloader.progress.complete(ProgressFinished, "sent to aria2")
		return nil
	}
	...
	if mergedFileExists {
		...
		downloader.progress.complete(ProgressSkipped, "file already exists")
		return nil
	}
	...
	if len(stream.Parts) == 1 {
		...
		downloader.progress.complete(ProgressFinished, "finished")
		return nil
	}
	...
	downloader.progress.setPhase(ProgressMerging, "merging", true)
